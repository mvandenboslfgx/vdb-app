...
| R-02 | Contract | Mobile contract was thinner than local migrations; RPC/naming drift | `contracts/backend-contract.json` now `0.1.1` / `2026.07.24.remediation` | P0 | High | Staging schema mismatch → runtime failures | **Yes** until owner publish | Conditionally | **Yes** | Local pin updated; owner must publish matching staging contract | CANONICAL_BACKEND_OWNER | Local mitigated — owner publish pending |
